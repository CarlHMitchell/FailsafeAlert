    ...
    public void onLoadFinished(Loader<Cursor> arg0, Cursor cursor) {
        ...
        do {
            ...
            if (mimeType.equals(CommonDataKinds.Phone.CONTENT_ITEM_TYPE)) {
                ...
                Log.d(TAG, cursor.getString(phoneColumnIndex));
                phoneNumbers.add(cursor.getString(phoneColumnIndex));
            } else if (mimeType.equals(CommonDataKinds.Email.CONTENT_ITEM_TYPE)) {
                //tv.append("\tEmail Address: " + cursor.getString(emailColumnIndex) + "\n");
                Log.d(TAG, cursor.getString(emailColumnIndex));
                emails.add(cursor.getString(emailColumnIndex));
            }
        ...
        Set<String> tempEmails = sharedPref.getStringSet("pref_emails", null);
        if (tempEmails != null) {
            emails.addAll(tempEmails);
        }
        editor.putStringSet("pref_emails", emails);
        editor.commit();
        Log.d(TAG, sharedPref.getStringSet("pref_emails", null).toString());
        Set<String> tempPhoneNumbers = sharedPref.getStringSet("pref_phone_numbers", null);
        if (tempPhoneNumbers != null) {
            phoneNumbers.addAll(tempPhoneNumbers);
        }
        editor.putStringSet("pref_phone_numbers", phoneNumbers);
        editor.commit();
        Log.d(TAG, sharedPref.getStringSet("pref_phone_numbers", null).toString());
    }
